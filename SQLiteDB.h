...
public:
	...
	SQLiteDB(std::string dbPath, bool deferInitAndClose = false, int mode = SQLITE_OPEN_READONLY);
	/// Destructor
	...
	/// @param sql The SQL statement
	/// @throws A string with an error message if the exec() fails.
	void exec(std::stringstream& sql);
...
protected:
	...
	void checkColumn(int colType, int col);
